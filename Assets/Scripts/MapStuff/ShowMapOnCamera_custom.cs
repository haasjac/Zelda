...
public class ShowMapOnCamera_custom : MonoBehaviour {


    static public ShowMapOnCamera_custom   S;
	static public int[,]		MAP;
	static public List<Tile_custom>	TILE_POOL;
    static public Tile_custom[,]       MAP_TILES; // This will largely remain empty
	...
	void Start () {
		...
		TILE_POOL = new List<Tile_custom>();

        MAP_TILES = new Tile_custom[w,h]; // Should fill with nulls - JB
    ...
    public void RedrawScreen(bool clearAll=false) {
        ...
        for (int i=i0 - tileClearOverage; i<i1 + tileClearOverage; i++) {
            for (int j=j0 - tileClearOverage; j<j1 + tileClearOverage; j++) {
                if (i<0 || j<0 || i>=ShowMapOnCamera_custom.S.w || j>= ShowMapOnCamera_custom.S.h ) { // Don't go out of bounds
                    continue;
                }
